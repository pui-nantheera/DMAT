...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="path of input folder containing videos",
    )
    parser.add_argument(
        "--out_folder",
        type=str,
        default=None,
        help="path of output folder to save processed videos",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    video_files = [
        f for f in os.listdir(args.input_folder) if f.endswith((".mp4", ".avi"))
    ]

    for video_file in video_files:
        vpath = os.path.join(args.input_folder, video_file)
        out_path = os.path.join(
            args.out_folder, os.path.splitext(video_file)[0] + ".mp4"
        )
        ...
        if args.save_video:
            if args.concatenate_input:
                ...
                for fid, frame in enumerate(out_frames):
                    output_writer.write(
                        np.concatenate((inp_frames[fid], frame), axis=1)
                    )
            else:
                output_writer = cv2.VideoWriter(
                    out_path, cv2.VideoWriter_fourcc("m", "p", "4", "v"), fps, (ww, hh)
                )
                for fid, frame in enumerate(out_frames):
