# This script implements a full video inference (using spatio-temporal patches)  of a trained 2 stage model
...
def split_to_patches(h, w, s):
    """Get position of a patch given height, width and patch size
    ...
    Returns:
        hpos (int): vertical patch position
        wpos (int): horizontal patch position
    """
    nh = h // s + 1
    nw = w // s + 1
...
def test_spatial_overlap(input_blk, model, patch_size):
    """Generate spatial overlap

    Args:
        input_blk (tensor): input block tensor
    ...
    for hi in hpos:
        for wi in wpos:
            ...
            out_masks[..., hi : hi + patch_size, wi : wi + patch_size].add_(
...
def restore_PIL(tensor, b, fidx):
    """extract numpy image arrays given frame and batch index
    ...
    Returns:
        img: numpy uint8 image array (range 0 to 255)
    """
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Video inference with overlapping patches"
    )
    parser.add_argument(
        "--patch_size",
        "-ps",
        ...
        type=int,
        default=256,
        help="saptial patch size",
    )
    parser.add_argument(
        "--temp_patch", type=int, default=12, help="temporal patch size"
    )
    parser.add_argument(
        "--resize_ratio",
        type=float,
        default=1.0,
        help="spatial resize ratio for both w and h",
    )
    parser.add_argument(
        "--start_frame",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="number of total frames to be processed",
    )
    parser.add_argument(
        "--input_path", type=str, default=None, help="path of input video"
    )
    parser.add_argument(
        "--out_path", type=str, default=None, help="path of output video"
    )
    parser.add_argument(
        "--model_path", type=str, default=None, help="Load model from a .pth file"
    )
    parser.add_argument(
        "--save_images", action="store_true", help="save results as images"
    )
    parser.add_argument(
        "--save_video", action="store_true", help="save results as video"
    )
    ...
    parser.add_argument(
        "--concatenate_input",
        action="store_true",
        help="concatenate input and output frames",
    )
...
if __name__ == "__main__":
    ...
    net.load_state_dict(
        checkpoint["state_dict"] if "state_dict" in checkpoint.keys() else checkpoint
    )
    ...
    if args.save_images:
        frame_dir = os.path.join(
            os.path.split(args.out_path)[0],
            os.path.split(args.out_path)[1].split(".")[0],
        )
    ...
    start_frame = (
        int(args.start_frame * total_frames)
    ...
    for i in range(1, num_chunk):
        if i == num_chunk - 1:
            test_frame_info.append(
                {
                    "start": total_frames - args.temp_patch,
                    "range": [
                        i * (args.temp_patch // 2) + (args.temp_patch // 4),
                        total_frames,
                    ],
                }
            )
    ...
    for i in range(num_chunk):
        ...
        in_range = [
            test_frame_info[i]["start"],
            test_frame_info[i]["start"] + args.temp_patch,
        ]
        inp_imgs = [
            Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
            for img in all_frames[in_range[0] : in_range[1]]
        ]
        ...
        if max(hh, ww) < patch_size:
            ...
            second_pass_output = test_spatial_overlap(
                first_pass_output, net, patch_size
            )
        ...
        for j in range(out_range[0] - in_range[0], out_range[1] - in_range[0]):
            ...
            if args.save_images:
                ...
                cv2.imwrite(
                    os.path.join(
                        frame_dir, "{:04d}.png".format(frame_idx + start_frame)
                    ),
                    oframe,
                )
            else:
    ...
    if args.save_video:
        if args.concatenate_input:
            ...
            output_writer = cv2.VideoWriter(
                args.out_path, cv2.VideoWriter_fourcc("m", "p", "4", "v"), fps, (ww, hh)
            )
            for fid, frame in enumerate(out_frames):
